<x-app-layout>
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h2 mb-0">Gerenciar Montadoras</h1>
            <a href="{{ route('admin.manufacturers.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-lg"></i>
                <span class="d-none d-sm-inline">Adicionar Nova Montadora</span>
            </a>
        </div>
                ...
                @foreach ($manufacturers as $manufacturer)
                    <div class="col">
                        <div class="card h-100 shadow-sm card-action"
                             ...
                             data-edit-url="{{ route('admin.manufacturers.edit', $manufacturer->id) }}"
                             data-destroy-url="{{ route('admin.manufacturers.destroy', $manufacturer->id) }}"
                             style="cursor: pointer;">
                            
                            <div class="card-body text-center d-flex align-items-center justify-content-center">
                                <h5 class="card-title mb-0">{{ $manufacturer->name }}</h5>
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="actionModalLabel">Ações para a Montadora</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body text-center">
                        <p>O que deseja fazer com esta montadora?</p>
                        <a href="#" id="modalEditButton" class="btn btn-secondary me-2">
                            <i class="bi bi-pencil-square"></i> Editar Montadora
                        </a>
                        <form action="#" method="POST" class="d-inline" id="modalDeleteForm" onsubmit="return confirm('Este é o último aviso. Tem a certeza que deseja excluir?');">
                            ...
                            <button type="submit" class="btn btn-danger">
                                <i class="bi bi-trash3-fill"></i> Excluir Montadora
                            </button>
                        </form>
                    </div>
                </div>
